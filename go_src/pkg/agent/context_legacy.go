...
// forceCompression reduces context when the limit is hit using a cache-friendly
// sliding-window strategy. Instead of dropping 50% in one shot (which breaks
// DeepSeek's prefix cache by changing the message array prefix), it drops only
// the oldest turn at a time. This preserves more of the message prefix for
...
func (m *legacyContextManager) forceCompression(sessionKey string) (compressionResult, bool) {
	...
	// Cache-friendly sliding window: drop only the oldest turn (1 cycle),
	// not 50%. This keeps the remaining prefix identical to what it was
	// before, minus one boundary — maximising the chance that DeepSeek's
	// prefix cache still hits for the rest.
	dropIdx := turns[0]
	if dropIdx <= 0 || dropIdx >= len(history) {
		return compressionResult{}, false
	}

	// Walk forward to a safe cut point (don't split a tool-call sequence
	// that crosses the turn boundary).
	cut := dropIdx
	...
	compressionNote := fmt.Sprintf(
		"[Dropped %d oldest messages (cache-friendly sliding window)]",
		droppedCount,
	)
	if existingSummary != "" {
	...
	logger.WarnCF("agent", "Cache-friendly compression", map[string]any{
		"session_key":    sessionKey,
		"dropped_msgs":   droppedCount,
		"remaining_msgs": len(keptHistory),
		"turns_before":   len(turns),
		"turns_after":    len(parseTurnBoundaries(keptHistory)),
	})
