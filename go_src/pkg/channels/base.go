...
type BaseChannel struct {
	...
	reasoningChannelID  string
	characterGetter     func() string // returns active character ID; "" = default persona
}
...
// SetCharacterGetter registers a function that returns the currently active character ID.
// When non-nil, character_id is injected into inbound message context for session isolation.
func (c *BaseChannel) SetCharacterGetter(fn func() string) {
	c.characterGetter = fn
}

// CharacterGetter returns the registered character getter, or nil.
func (c *BaseChannel) CharacterGetter() func() string {
	return c.characterGetter
}
...
func (c *BaseChannel) HandleMessageWithContext(
	...
	// Inject active character ID for session dimension isolation.
	// All channels inherit this through BaseChannel automatically.
	if c.characterGetter != nil {
		if charID := c.characterGetter(); charID != "" {
			if inboundCtx.Raw == nil {
				inboundCtx.Raw = make(map[string]string)
			}
			inboundCtx.Raw["character_id"] = charID
		}
	}
