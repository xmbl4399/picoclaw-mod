...
多层级缓存体系，针对 DeepSeek 自动前缀缓存 + Anthropic per-block cache_control 优化：

```
System Prompt Cache (ContextBuilder)
  +-- epoch 快速路径 ---- 避免每次读命中 stat 遍历  --> 延迟 ↓
  
LLM Prompt Cache
  +-- static prompt 保持在 message 前缀 ---- DeepSeek 前缀命中率 ↑
  +-- summary 前缀拆独立缓存块 ---- Anthropic KV cache 复用
  +-- prompt_cache_key ---- OpenAI/Azure 客户端缓存

Seahorse 短时记忆引擎
  +-- FreshTail 按 token 比例计算 (20%) ---- 大窗口保护更多上下文
  +-- 叶压缩 + 凝聚压缩 ---- 多层摘要降级
  +-- FTS5 trigram 全文搜索 ---- CJK 支持
```
...
- 选项按钮可点击，自动填充消息（已修复防连点）
- Markdown 渲染 + Token 计数器 + 流式输出 + GAL format 全链路贯通
...
| `pkg/seahorse/short_constants.go` | FreshTailTokenPercent 20% 按比例保护 |
| `pkg/seahorse/short_assembler.go` | token-budget-aware FreshTail 计算 |
